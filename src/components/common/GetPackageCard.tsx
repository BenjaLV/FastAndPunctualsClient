...
const GetPackageCard = ({ text }: Props) => {
  ...
  return (
    ...
    >
      <label
        className={`relative mr-2 ml-2 h-[14px] w-[14px] flex items-center cursor-pointer ${
          isChecked ? "bg-alternative" : "bg-white"
        }`}
      >
        <input
          type="checkbox"
        ...
        />
        <div
          className={`absolute top-0 left-0 w-full h-full border border-textColor ${
            isChecked ? "bg-alternative" : "bg-white"
          }`}
        />
        {isChecked && (
